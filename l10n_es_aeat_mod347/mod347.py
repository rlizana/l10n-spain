...
class L10nEsAeatMod347Report(orm.Model):
    ...
    def _calculate_cash_records(self, cr, uid, partner, partner_ids,
        ...
        received_cash_amount = sum([line.credit
        ...
        if received_cash_amount > report.received_cash_limit:
            ...
            for cash_move_fy_id in cash_moves.keys():
                receivable_amount = sum(
                    [line.credit for line in cash_moves[cash_move_fy_id]])
                if receivable_amount > report.received_cash_limit:
                    if (cash_move_fy_id != report.fiscalyear_id.id
                       or not partner_record_id):
        ...
        for report in self.browse(cr, uid, ids, context):
            ...
            if report.only_supplier:
                partner_ids = partner_obj.search(
                    cr, uid,
                    [('not_in_mod347', '=', False), ('supplier', '=', True)])
            else:
                partner_ids = partner_obj.search(
                    cr, uid,
                    [('not_in_mod347', '=', False),
        ...
        for report in self.browse(cr, uid, ids, context=context):
            res[report.id] = {
                ...
                'total_real_state_transmissions_amount': sum(
                    [record.real_state_transmissions_amount for record in
    ...
    _columns = {
        ...
        'received_cash_limit': fields.float('Received cash Limit (2)',
                                            ...
                                            " over this limit"),
        'charges_obtp_limit': fields.float('Charges on behalf of third parties '
                                           'Limit (3)',
                                           digits=(13, 2),
                                           help="The declaration will include"
    ...
    def button_confirm(self, cr, uid, ids, context=None):
        ...
        for item in self.browse(cr, uid, ids, context):
            # Browse partner record lines to check if all are correct
            # (all fields filled)
...
class L10nEsAeatMod347PartnerRecord(orm.Model):
    ...
    def _get_real_state_record_ids(self, cr, uid, ids, field_name, args,
        ...
        """
        res = {}
        real_state_record_obj = self.pool[
            'l10n.es.aeat.mod347.real_state_record']
    ...
    def _set_real_state_record_ids(self, cr, uid, field_name, values,
        ...
        if context is None:
            context = {}
        if values:
            real_state_record_obj = self.pool.get(
                'l10n.es.aeat.mod347.real_state_record')
            for value in values:
    ...
    _columns = {
        ...
        'operation_key': fields.selection([
            ('A', u'A - Adquisiciones de bienes y servicios superiores '
             u'al límite (1)'),
            ('B', u'B - Entregas de bienes y servicios superiores '
             u'al límite (1)'),
            ('C', u'C - Cobros por cuenta de terceros superiores '
             u'al límite (3)'),
    ...
    def on_change_partner_id(self, cr, uid, ids, partner_id):
        ...
        if partner_id:
            ...
            # Get the default invoice address of the partner
            address = None
            address_ids = self.pool.get('res.partner').address_get(
            ...
            partner_vat = partner.vat and re.match("(ES){0,1}(.*)",
                                                   partner.vat).groups()[1]
            partner_state_code = (
                address.state_id and address.state_id.code or '')
            partner_country_code = (
...
class L10nEsAeatMod347RealStateRecord(orm.Model):
    ...
    _defaults = {
        ...
        'representative_vat': lambda self, cr, uid, context=None: (
            context and context.get('representative_vat', None)),
    }
